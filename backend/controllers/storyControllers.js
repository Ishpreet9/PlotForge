...
// This single function handles all Gemini API calls and response parsing
const generateContentFromGemini = async (prompt) => {
  try {
    ...
    throw new Error('Error generating content from Gemini');
  }
};

const generateContentFromGroq = async (prompt) => {
  try {
    if (!prompt) {
    ...
    console.log(error);
    throw new Error('Error generating content from groq');
  }
}

const initialIdeas = async (req, res) => {
  try {
    const prompt = `Generate 5-7 unique and creative ideas for different stories and return them as a JSON array.
    Each idea should provide a short overview of the story. 
    Example output format: ["A world where dreams can be traded as currency", "A detective who solves crimes through memories"]. 
    Make sure the output is a valid JSON array with exactly 5 story ideas and do not use special characters.
    Rules:
- Return ONLY a valid JSON array with exactly 5 strings.
- Each string must be a short description (max 12 words).
- Use only letters, numbers, and spaces (no punctuation, no special characters).
- Do not include explanations, headings, or code fences.`;

    const finalArray = await generateContentFromGroq(prompt);
...
6. Example: Original story idea: "A story of a child and a dog". Similar story ideas: ["A child and their dog find a hidden path in the forest and explore it together"].
Now, generate 5 similar story ideas for: "${userIdea}". Return ONLY the JSON array.`;

    const finalArray = await generateContentFromGroq(prompt);
    res.status(200).json({ success: true, message: finalArray });
  } catch (error) {
    res.status(500).json({ message: 'Error generating similar story ideas' });
  }
};
...
4. Output must be a valid JSON array of strings ONLY.
5. Example: Original story idea: "A story of a child and a dog". Possible initial events: ["The child wakes up to find their dog missing from the backyard"].
Now, generate 5-7 possible initial events for: "${initialIdea}". Return ONLY the JSON array.`;

    const finalArray = await generateContentFromGroq(prompt);
    res.status(200).json({ success: true, message: finalArray });
  } catch (error) {
    res.status(500).json({ message: 'Error generating initial choices' });
  }
};
...
7. Example: Story so far: ["The child finds a mysterious key in the attic", "The dog starts barking at the locked basement door"]. Possible next events: ["The child tries the key on the basement door", "The dog scratches the floor, uncovering a hidden trapdoor"].
Now, generate 5-7 possible next events for: ${stEvents}. Return ONLY the JSON array.`;

    const finalArray = await generateContentFromGroq(prompt);
    res.status(200).json({ success: true, message: finalArray });
  } catch (error) {
    res.status(500).json({ message: 'Error generating next choices' });
  }
};
...
Example: Events: ["The child finds a mysterious key in the attic"]. Output: ["The Secret Key", "One rainy afternoon, a curious child stumbled upon a mysterious key in the dusty attic."].
Now, generate the JSON array (title + story) for these events: ${stEvents}. Return ONLY the JSON array.`;
